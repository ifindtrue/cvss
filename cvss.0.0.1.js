	...
	(function(window){
		...
		Canvas.prototype.canvasMouseOver=function(event){
			...
			for(i=this.$elementKey.length-1; i>=0; i--)
			{
				...
				if(insideCheck){
					...
					this.$canvas.style.cursor=element.cursor || Canvas.prototype.$initElement.cursor;
		...
		Canvas.prototype.createElement=function (info,/*optional*/option){
			...
			var info_key=Object.keys(info),element,img_obj=[],img_info=[],
				initElement=Canvas.prototype.$initElement, // the initial value of cvssElement
			...
			for(i=0; i<info_key.length; i++){
				...
				switch(element.type){
					case "block":
						var x=element.x,y,width,height,temp,
							border={width:0,color:0},sourceX,sourceY,src,pattern;
						...
						if(element.$temp || !$element){
							...
							element.x=numberFormat(persentFormat(this.$canvas.x,element.x,initElement.x),($element && $element.x) || 0,initElement.x);
							element.y=numberFormat(persentFormat(this.$canvas.y,element.y,initElement.y),($element && $element.y) || 0,initElement.y);
							...
							x=element.x;
							y=element.y;
							var	origin=typeof element.origin=="string" ? element.origin.split(" ") : initElement.origin;
							...
							if((!$element && typeof element.border=="string")|| typeof element.border=="string" && ($element && $element.border!=element.border)){
								var borderArr=element.border.split(" ");
						...
						if( element.onlyBaseElement===true || element.onlyBaseElement=="true"){
							...
							this.$element[info_key[i]]=element;
							this.$context.restore();continue;												
						}
						if(element.borderWidth>0){this.$context.strokeRect(x-element.borderWidth/2,y-element.borderWidth/2,width+element.borderWidth,height+element.borderWidth);}
					...
					case "circle":
						if(element.$temp || !$element){
							var x=numberFormat(element.x,($element && $element.x) || 0,initElement.circle.x);
							var y=numberFormat(element.y,($element && $element.y) || 0,initElement.circle.y);
							var isR1=(!$element && element.r1) || ($element && ($element.r1!=element.r1));
							var isR2=(!$element && element.r2) || ($element && ($element.r2!=element.r2));
							...
							if(!element.r1){
								element.r1=initEelemnt.circle.r;
							}
							if(!element.r2){
							...
							if(typeof element.origin=="object"){
								x-=persentFormat(element.r1*2,element.origin[0],initElement.origin[0]);
								y-=persentFormat(element.r1*2,element.origin[1],initElement.origin[1]);
							}					
					...
					case "text":
						...
						var fontTemp=(element.fontSize || initElement.fontSize)+" "+(element.fontFamily || initElement.fontFamily);
